//--------------------------------------------- // 
#include "precompiledHeaders.h"                 // <--- this include is necessary in order to use precompiled headers in visual studio.
...
{                                               //      increment later if you add properties and still want to read old example files
   ...
   ADD_NOARGS_METHOD(ExampleExperiment::matrixVectorProduct)       // <--- register method 3
	ADD_TIMER_METHOD_EX(ExampleExperiment::outputCounter, 200)      // <--- register a timer method; default interval between calls is set to 200ms. The second argument is optional (remove the "_EX" from the macro then)
}                                               //
                                                //
...
void ExampleExperiment::outputCounter()
{
	// ugly global variable; just for the example (not to spoil the public members)
