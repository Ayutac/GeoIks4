...
template <class FloatType, unsigned dim>
inline void StaticVector<FloatType, dim>::setZero() {
   for (unsigned i=0; i<dim; i++) {
      v[i] = (FloatType)0;
   }
}
...
template <class FloatType, unsigned columns, unsigned rows>
inline void StaticMatrix<FloatType, columns, rows>::setZero() {
   for (unsigned i=0; i<columns; i++) {
...
template <class FloatType, unsigned columns, unsigned rows>
void StaticMatrix<FloatType, columns, rows>::computeEigenStructure( StaticVector<FloatType,rows>& eigenValues, StaticMatrix<FloatType,rows,columns>& eigenVectors ) const
{
   if ( rows != columns ){
...
inline Matrix3f expand2To3(Matrix3f v) {
	Matrix3f result;
	for (int x = 0; x<2; x++) {
		for (int y = 0; y<2; y++) {
			result[x][y] = v[x][y];
...
inline Vector2f shrink3To2(Vector3f v) {
	return makeVector2f(v[0], v[1]);
}

inline Matrix2f shrink3To2(Matrix3f v) {
	Matrix2f result;
	for (int x = 0; x<2; x++) {
		for (int y = 0; y<2; y++) {
			result[x][y] = v[x][y];
...
inline Vector2f transformVector2f(const Matrix3f &M, const Vector2f &v) {
	return shrink3To2(M*(expand2To3(v)));
}
