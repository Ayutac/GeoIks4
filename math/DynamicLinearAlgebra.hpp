...
inline DynamicVector<FloatType>::DynamicVector(mpcard dim) {
   this->dim = dim;
   v = new FloatType[dim];
}

template <typename FloatType>
inline mpcard DynamicVector<FloatType>::getDim() const {
   return dim;
}

template <typename FloatType>
inline mpcard DynamicVector<FloatType>::size() const {
   return dim;
}

template <typename FloatType>
inline void DynamicVector<FloatType>::setDim(const mpcard dim) {
   this->dim = dim;
   delete[] v;
   v = new FloatType[dim];
}
...
template <typename FloatType>
inline FloatType& DynamicVector<FloatType>::operator[](const mpcard &index) {
...
template <typename FloatType>
inline const FloatType& DynamicVector<FloatType>::operator[](const mpcard &index) const{
   #ifdef DVECT_RANGE_CHECK
   if (index >= dim) {
      throw ERangeCheck("DynamicVector::operator[] - index exceeds bounds");
   }
...
template <typename FloatType>
inline DynamicVector<FloatType> DynamicVector<FloatType>::copyElements(const mpcard &howMany) const {
   #ifdef DVECT_RANGE_CHECK
   if (howMany > dim) {
...
template <typename FloatType>
inline void DynamicVector<FloatType>::setDataPointer(FloatType* x, const mpcard dim){
   delete[] v;
   this->dim = dim;
   v=x;
}
...
template <typename FloatType>
inline DynamicMatrix<FloatType>::DynamicMatrix( mpcard c, mpcard r, bool initAsIdentity) {
   columns = c;
   rows = r;
   theColumns = new DynamicVector<FloatType>[columns];
	for ( mpcard n = 0; n < columns; n++ )
      theColumns[n] = DynamicVector<FloatType>(rows);
	for (mpcard j=0; j<columns; j++) {
...
template <typename FloatType>
inline void DynamicMatrix<FloatType>::setDimension(mpcard columns, mpcard rows, bool initAsIdentity) {
   delete[] theColumns;
   this->rows = rows;
...
template <typename FloatType>
inline mpcard DynamicMatrix<FloatType>::getRowsDim() const{
     return rows;
}

template <typename FloatType>
inline mpcard DynamicMatrix<FloatType>::getColsDim() const{
     return columns;
}
...
template <typename FloatType>
inline mpcard DynamicMatrix<FloatType>::getRows() const {
   return rows;
}

template <typename FloatType>
inline mpcard DynamicMatrix<FloatType>::getColumns() const {
   return columns;
}
...
inline DynamicMatrix<FloatType>::DynamicMatrix(const DynamicMatrix<FloatType> &init) {
   ...
   this->rows = init.rows;
   theColumns = new DynamicVector<FloatType>[columns];
	for (mpcard i=0; i<columns; i++) {
      theColumns[i] = init.theColumns[i];
   }
}

template <typename FloatType>
inline DynamicVector<FloatType>& DynamicMatrix<FloatType>::operator[](const mpcard &index) {
   #ifdef DVECT_RANGE_CHECK
   if (index >= columns) {
...
template <typename FloatType>
inline const DynamicVector<FloatType>& DynamicMatrix<FloatType>::operator[](const mpcard &index) const {
   #ifdef DVECT_RANGE_CHECK
   if (index >= columns) {
      throw ERangeCheck("DynamicMatrix::operator[] - index exceeds bounds");
   }
...
inline DynamicMatrix<FloatType> DynamicMatrix<FloatType>::operator*(const DynamicMatrix<FloatType> &op) const {
   ...
   DynamicMatrix<FloatType> result( op.columns, rows );
	for ( mpcard r = 0; r < rows; r++ ){
		for ( mpcard c = 0; c < op.columns; c++ ){
         result[c][r] = 0.0;
			for ( mpcard i = 0; i < columns; i++ ) {
...
inline DynamicVector<FloatType> DynamicMatrix<FloatType>::operator*(const DynamicVector<FloatType> &v) const {
   ...
   DynamicVector<FloatType> result(rows);
	for (mpcard y=0; y<rows; y++) {
      result[y] = 0.0;
		for (mpcard x=0; x<columns; x++) {
...
template <typename FloatType>
inline void DynamicMatrix<FloatType>::changeRows(mpcard row1, mpcard row2) {
...
template <typename FloatType>
inline void DynamicMatrix<FloatType>::multRow(mpcard row, FloatType value) {
	for (mpcard i=0; i<columns; i++) {
...
template <typename FloatType>
inline void DynamicMatrix<FloatType>::combineRows(mpcard row, mpcard with, FloatType by) {
	for (mpcard i=0; i<columns; i++) {
...
template <typename FloatType>
inline void DynamicMatrix<FloatType>::addRows(mpcard row, mpcard with) {
	for (mpcard i=0; i<columns; i++) {
...
template <typename FloatType>
inline DynamicMatrix<FloatType> DynamicMatrix<FloatType>::copyColumns(mpcard startColumn, mpcard numColumns) {
   pAssert(startColumn < columns);
   pAssert(startColumn + numColumns < columns);
   DynamicMatrix<FloatType> result(numColumns, rows, false);
	for (mpcard i=0; i<numColumns; i++) {
...
template <typename FloatType>
inline FloatType DynamicMatrix<FloatType>::maxComponent() const
{
	if (columns>0 && rows>0) {
		FloatType currentMax = theColumns[0].maxComponent();
		for (mpcard i=1; i<columns; i++) {
			FloatType comp = theColumns[i].maxComponent();
			if (currentMax < comp) currentMax = comp;
		}
		return currentMax;
	} else {
		throw EInvalidState("empty matrix");
	}
}

template <typename FloatType>
inline FloatType DynamicMatrix<FloatType>::minComponent() const
{
	if (columns>0 && rows>0) {
		FloatType currentMin = theColumns[0].minComponent();
		for (mpcard i=1; i<columns; i++) {
			FloatType comp = theColumns[i].minComponent();
			if (currentMin > comp) currentMin = comp;
		}
		return currentMin;
	} else {
		throw EInvalidState("empty matrix");
...
inline void convertDVectorType(const DynamicVector<FloatType1> &source, DynamicVector<FloatType2> &dest) {
   if (source.getDim() != dest.getDim()) {
	...
	for (mpcard i=0; i<source.getDim(); i++) {
...
inline DynamicMatrix<FloatType> outerProduct( const DynamicVector<FloatType> &v1,const DynamicVector<FloatType> &v2 )
{
	...
	for( mpcard i=0;i<v1.getDim();i++ )
	{
		for( mpcard j=0;j<v2.getDim();j++ )
		{
			ret[i][j] = v1[i] * v2[j];
		}
	}
...
inline FloatType normInfinity(const DynamicVector<FloatType> &v){
	FloatType max = -1e10;
	for(mpcard index = 0; index < v.getDim(); index++)
	{
		if(fabs(v[index]) > max)
		{
			max = fabs(v[index]);
...
inline DynamicMatrix<FloatType> invertMatrix(const DynamicMatrix<FloatType> &mat) {
   ...
   DynamicMatrix<FloatType> mCopy = mat;
	for (mpcard step=0; step<size; step++) {
		mpcard pivot = searchPivot(mCopy, step);
      if (step != pivot) {
      ...
      result.multRow(step, m);
		for (mpcard row=step+1; row<size; row++) {
