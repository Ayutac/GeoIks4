...
card64 Timer::getCurrentValue() {
	return timer.elapsed();
}

card64 Timer::getDeltaValue(card32 lastValue) {
   return timer.elapsed() - lastValue;
}

card64 Timer::getDeltaValue() {
	card64 t = timer.elapsed();
	card64 result = t - lastTime;
   lastTime = t;
   return result;
}
