...
ClassTypeClassPropertyEditorWidget::ClassTypeClassPropertyEditorWidget(QWidget * parent, Qt::WindowFlags f)
...
void ClassTypeClassPropertyEditorWidget::collectClassList(const string baseClass,QStringList &list )
{
	...
	for( card32 i=0;i<mc->getNumDescendants();i++ )
	{
		...
		if (!mc->getDescendant(i)->isAbstract()) list << className.c_str();
