...
*/
 ...
 public:
   ...
   virtual void readProperty(Object* obj, pointer_diff offset, InputObjectStream *in) const;
	...
	virtual string toString(const Object* obj, pointer_diff offset) const {UNUSED(obj); UNUSED(offset); return "";}
...
#define IMPLEMENT_OBJECT_LIST_ACCESS_STL(propName,cName) \
   void add##propName ( Object * obj ) {propName .push_back( ( cName *) obj );} \
   void delete##propName##s ( mpcard begin, mpcard end ) {\
   for( mpcard i=begin;i<end;i++ ) delete propName [i];  propName . erase( propName . begin() + begin, propName . begin() + end ); \
} \
   Object * get##propName ( mpcard index ) {return static_cast<cName * >(propName [index]);} \
   void move##propName ( mpcard oldPos, mpcard newPos ) {\
   cName * tmp = static_cast<cName *>(propName [oldPos]); \
   propName . erase(propName . begin() + oldPos); \
   propName . insert(propName . begin() + newPos, tmp); \
} \
   mpcard getNum##propName##s() {return propName . size(); } \
   string get##propName##Name(mpcard index ) {return intToStr(index);}


#define ADD_OBJECT_LIST_PROP( myClassName, propName, version, metaClass ) \
	((MetaClass*)getClass())->addProperty( new ObjectListProperty( #propName , MetaClass::FIRST_VERSION + version, \
	metaClass, (ObjectListProperty::AddObjectMethod) & myClassName :: add##propName, \
	(ObjectListProperty::DeleteObjectsMethod)& myClassName :: delete##propName##s , \
	(ObjectListProperty::GetObjectMethod)& myClassName :: get##propName , \
	(ObjectListProperty::MoveObjectMethod)& myClassName :: move##propName , \
	(ObjectListProperty::GetNumObjectsMethod)& myClassName :: getNum##propName##s , \
	(ObjectListProperty::GetObjectNameMethod)& myClassName :: get##propName##Name ) );
